...
async def chat(request: ChatRequest, current_user: User = Depends(get_current_user)):
    ...
    results = search_documents(
        rewritten_query, user_id=current_user.id, top_k=TOP_K, source=request.source
    )
    if not results:
    ...
    answer = await (_answer_prompt | llm).ainvoke(
        {"question": request.question, "context": context}
    )

    sources = [
        ChatSource(
            file_name=file_name, file_path=chunks[0].file_path, source=chunks[0].source
        )
