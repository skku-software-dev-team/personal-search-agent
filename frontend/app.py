...
def home():
    ...
    with st.sidebar:
        ...
        goal = st.text_input(
            "목표",
            value=st.session_state.get("s_goal", ""),
            placeholder="예: ML 엔지니어, 백엔드 개발자",
        )
        fields = st.multiselect(
            "관심 분야",
            options=FIELD_OPTIONS,
            default=st.session_state.get("s_fields", []),
        )
        level = st.radio(
            "현재 레벨",
            LEVELS,
            index=LEVELS.index(st.session_state.get("s_level", "취준생")),
            horizontal=True,
        )
        timeline = st.select_slider(
            "목표 기간",
            options=TIMELINES,
            value=st.session_state.get("s_timeline", "6개월"),
        )
    ...
    if st.session_state.get("s_goal"):
        st.info(
            f"🎯 현재 목표: **{st.session_state['s_goal']}** ({st.session_state['s_level']} · {st.session_state['s_timeline']})"
        )
    else:
    ...
    with col1:
        with st.container(border=True):
            ...
            st.page_link(
                "pages/1_gaps.py", label="분석 시작 →", use_container_width=True
            )
    ...
    with col2:
        with st.container(border=True):
            ...
            st.markdown(
                "기간별로 어떤 주제를 공부했는지 " "시간 흐름에 따라 시각화합니다."
            )
            st.page_link(
                "pages/2_timeline.py", label="타임라인 보기 →", use_container_width=True
            )
    ...
    with col4:
        with st.container(border=True):
            st.markdown("### 💬 문서와 대화")
            st.markdown("내 문서를 RAG로 검색해 " "질문에 근거 기반으로 답변합니다.")
            st.page_link(
                "pages/4_chat.py", label="채팅 시작 →", use_container_width=True
            )
    ...
    with col5:
        with st.container(border=True):
            st.markdown("### 🔍 문서 검색")
            st.markdown(
                "키워드가 아닌 의미 기반 벡터 검색으로 " "관련 문서를 빠르게 찾습니다."
            )
            st.page_link(
                "pages/5_search.py", label="검색하기 →", use_container_width=True
            )
...
pg = st.navigation(
    [
        ...
        st.Page("pages/1_gaps.py", title="지식 공백 분석", icon="🕳️"),
        st.Page("pages/2_timeline.py", title="지적 성장 타임라인", icon="📈"),
        st.Page("pages/3_portfolio.py", title="포트폴리오 생성", icon="👤"),
        st.Page("pages/4_chat.py", title="문서와 대화", icon="💬"),
        st.Page("pages/5_search.py", title="문서 검색", icon="🔍"),
    ]
)
